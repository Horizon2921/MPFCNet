...
class LKA(nn.Module):
    def __init__(self, dim, stride=1):
        ...
        self.proj1 = nn.Conv3d(dim, dim, kernel_size=1)
        self.act = nn.GELU()
...
class HighResolutionNet(nn.Module):
    def __init__(self, base_channel: int = 32, output_channels: int = 4):
        ...
        # Final layer
        self.attn = LKA(base_channel)
        # self.van = VANBlock(base_channel,  mlp_ratio=4., drop=0., drop_path=0., act_layer=nn.GELU)
    ...
    def forward(self, x):
        ...
        # 查看stage4输出的torch.shape，会发现只保留了最大的尺寸，即layer1的尺寸，其他尺寸已经通过上采样融合到layer1
        # for idx in x:
        #     print("here",idx.shape)
        x = x[0] # 此步将stage4输出的list[0]转换为torch，方便卷积运算。因为stage输出的List只有一个数据，只取list[0]
        ...
        x = self.up(x)
        # print('before final_layer x shape', x[0].shape)
        # before final_layer x shape torch.Size([2, 32, 96, 96, 96])
        x = self.final_layer(torch.cat((x, residual),dim=1))


        # print('x shape', x.shape)
        # x shape torch.Size([2, 4, 96, 96, 96])
        return x
