...
from utils.dataloader_spacing import MedData_train, MedData_test, Data_fetcher
...
def train():
    ...
    optimizer1 = torch.optim.SGD(model1.parameters(), lr=args.init_lr, momentum=0.9, weight_decay=1e-4)
    ...
    # criterion = DiceCELoss(include_background=True,to_onehot_y=True,softmax=True).cuda()
    ...
    un_util = MedData_train(unlabeled_image_dir, unlabeled_label_dir, False, batch_size=batch_size - labeled_bs)
    ...
    # val_util = MedData_test(labeled_image_dir,labeled_label_dir,)
    # val_loader = val_util.test_loader

    val_util = MedData_test(val_image_dir, val_label_dir,unlabeled=False )
    val_loader = val_util.test_loader
    ...
    for epoch in range(1, epochs+1):
        ...
        #     print(f"Batch: {i}/{ites_per_epoch} epoch {epoch}")
            ...
            outputs_tanh1, outputs1 = model1(x,False, True)
            supervised_loss1 = criterion(outputs1, y)
            supervised_loss = supervised_loss1
            loss = supervised_loss

            y_pred = make_one_hot(torch.argmax(torch.softmax(outputs1.detach(), dim=1), dim=1, keepdim=True),hp.out_class)
            y_ = make_one_hot(y.long(),hp.out_class)
        ...
        print(f"epoch {epoch} average loss: {epoch_loss:.4f} average dice: {Train_dice:.4f} ")
        ...
        # if epoch % 1 == 0:
            ...
            with torch.no_grad():
                ...
                for val_data in val_loader:
                    ...
                    val_labels = [post_label(i) for i in decollate_batch(
                        val_labels)]
                    dice_metric(y_pred=val_outputs, y=val_labels)
                    dice_metric_channel(y_pred=val_outputs, y=val_labels)
                    if save_flag ==1:
                        save_flag=0
                        saver1 = SaveImage(output_dir=output_dir_test, output_ext=".nii.gz", output_postfix="seg",print_log=False)
                        saver2 = SaveImage(output_dir=output_dir_test, output_ext=".nii.gz", output_postfix="ori",print_log=False)
                        for val_ori,val_output in zip(val_inputs,val_outputs):
