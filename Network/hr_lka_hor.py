...
class gnconv(nn.Module):
    def __init__(self, dim, order=3, gflayer=None, h=14, w=8, s=1.0):
        ...
        if gflayer is None:
            ...
            self.dwconv = nn.Conv3d(dim, dim, kernel_size=7, padding=(7 - 1) // 2,  groups=dim)
        else:
    ...
    def forward(self, x, mask=None, dummy=False):
        ...
        #print('gnconv_fusedx', fused_x.shape)
...
class HorBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # print('horblock,x.shape1', x.shape)
        x = self.norm2(x)
        x = self.pwconv1(x)
        x = self.act(x)
        x = self.pwconv2(x)
        ...
        # print('horblock,x.shape2', x.shape)
