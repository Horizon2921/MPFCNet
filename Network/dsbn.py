...
class SPADE(nn.Module):
    def __init__(self,  inplanes,  ks=3, param_free_norm_type='BN', spade_seg_mode='soft',
        ...
        if param_free_norm_type == 'BN':
            self.param_free_norm = nn.BatchNorm3d(inplanes, affine=False)


        # define mlp_shared
        self.mlp_shared = nn.Sequential(
                nn.Conv3d(inplanes , inplanes, kernel_size=3, padding=1),
                nn.ReLU()
            )
        # define mlp_gamma/beta
        self.mlp_gamma = nn.Conv3d(inplanes, inplanes, kernel_size=1, padding=0)
        self.mlp_beta = nn.Conv3d(inplanes, inplanes, kernel_size=1, padding=0)
        self.spade_seg_mode = spade_seg_mode
        self.spade_inferred_mode = spade_inferred_mode

    def forward(self, x, segmap=True):
        # Part 1. generate parameter-free normalized activations
        normalized = self.param_free_norm(x)
        # Part 2. produce scaling and bias conditioned on semantic map
        # print('normalized', normalized.shape)
        segmap = F.interpolate(x.float(), size=x.size()[2:], mode='trilinear', align_corners=True)
...
class BasicBlockSPADE(nn.Module):
    expansion = 1
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        return out
class BottleneckSPADE(nn.Module):
    expansion = 4

    def __init__(self, inplanes, planes, stride=1, downsample=None):
        super(BottleneckSPADE, self).__init__()
        self.conv1 = nn.Conv3d(inplanes, planes, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm3d(planes, momentum=BN_MOMENTUM)
        self.conv2 = nn.Conv3d(planes, planes, kernel_size=3, stride=stride,
                               padding=1, bias=False)
        self.bn2 = SPADE(planes, planes)
        self.conv3 = nn.Conv3d(planes, planes * self.expansion, kernel_size=1,
                               bias=False)
        self.bn3 = nn.BatchNorm3d(planes * self.expansion,
                                  momentum=BN_MOMENTUM)
...
class HighResolutionNetLKASPAde(nn.Module):
    def __init__(self, base_channel: int = 32, output_channels: int = 6):
        ...
        self.van = VANBlock(base_channel)
...
if __name__ == '__main__':
    ...
    network = HighResolutionNetLKASPAde()
