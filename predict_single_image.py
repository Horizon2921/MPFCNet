...
from utils import *
import argparse
from Network.networks import build_net, update_learning_rate, build_UNETR, build_unet, build_vnet, build_swinUNet
...
def segment(image, label, result, weights, resolution, patch_size, network, gpu_ids):
    ...
    with torch.no_grad():
        ...
        if resolution is not None:
            ...
            writer.SetFileName('temp_seg.nii.gz')
            writer.Execute(result_array_temp)

            files = [{"image": 'temp_seg.nii.gz'}]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--image", type=str, default='/home/dluser/dataset/nnUNetFrame/DATASET/nnUNet_raw/nnUNet_raw_data/Task55_k/imagesTs/9410340.nii.gz', help='source image' )
    parser.add_argument("--label", type=str, default='/home/dluser/dataset/nnUNetFrame/DATASET/nnUNet_raw/nnUNet_raw_data/Task55_k/labelsTs/9410340.nii.gz', help='source label, if you want to compute dice. None for new case')
    parser.add_argument("--result", type=str, default='/home/dluser/dataset/nnUNetFrame/DATASET/nnUNet_raw/nnUNet_raw_data/Task55_k/output_save', help='path to the .nii result to save')
    parser.add_argument("--weights", type=str, default='/home/dluser/dataset/nnUNetFrame/Pytorch--3D-Medical-Images-Segmentation/best_metric_model_test_0818_swinunet.pth', help='network weights to load')
    parser.add_argument("--resolution", default=[0.7, 0.365, 0.365], help='Resolution used in training phase')
    parser.add_argument("--patch_size", type=int, nargs=3, default=(96,96,96), help="Input dimension for the generator, same of training")
    parser.add_argument('--network', default='swinUNetR', help='nnunet, unetr, unet, vnet')
