...
if __name__ == "__main__":
    ...
    train_images = sorted(glob(os.path.join(opt.images_folder, '*.nii.gz')))
    train_segs = sorted(glob(os.path.join(opt.labels_folder, '*.nii.gz')))
    ...
    loader = DataLoader(check_ds, batch_size=1, shuffle=False, num_workers=0, pin_memory=torch.cuda.is_available())
    check_data = monai.utils.misc.first(loader)
    im, seg, coord1, coord2 = (check_data['image'][1], check_data['label'][1],check_data['foreground_start_coord'][0],
