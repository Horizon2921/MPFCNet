...
def main():
    ...
    # 在训练过程中保存预测结果
    saver_ori = SaveImage(output_dir='/home/dluser/dataset/nnUNetFrame/Pytorch--3D-Medical-Images-Segmentation/result2023/nnunet', output_ext=".nii.gz", output_postfix="ori",print_log=True)
    saver_gt = SaveImage(output_dir='/home/dluser/dataset/nnUNetFrame/Pytorch--3D-Medical-Images-Segmentation/result2023/nnunet', output_ext=".nii.gz", output_postfix="gt",print_log=True)
    saver_seg = SaveImage(output_dir='/home/dluser/dataset/nnUNetFrame/Pytorch--3D-Medical-Images-Segmentation/result2023/nnunet', output_ext=".nii.gz", output_postfix="seg",print_log=True)
    ...
    ckpt = torch.load(
        '/home/dluser/dataset/nnUNetFrame/Pytorch--3D-Medical-Images-Segmentation/checkpoint/best_metrich_1018_monai_nnunet.pth')
        ...
        if (epoch + 1) % 5 == 0:
            torch.save(
                {"net": net.state_dict(),
                 ...
                 "epoch": epoch},
                os.path.join(opt.output_folder, 'latest_checkpoint_0305_monai_nnunet.pth')
            )
                ...
                if dice > best_metric:
                    ...
                    torch.save(
                        { "net": net.state_dict(),
                          "optim": optimizer.state_dict(),
                          "epoch": epoch},
                        os.path.join(opt.output_folder, 'best_metric_0305_monai_nnunet.pth')
                    )
                print('')
