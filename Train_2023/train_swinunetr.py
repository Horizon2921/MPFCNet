...
def main():
    ...
    # 在训练过程中保存预测结果
    saver_ori = SaveImage(output_dir='/home/dluser/dataset/nnUNetFrame/Pytorch--3D-Medical-Images-Segmentation/result2023/swinunetr', output_ext=".nii.gz", output_postfix="ori",print_log=True)
    saver_gt = SaveImage(output_dir='/home/dluser/dataset/nnUNetFrame/Pytorch--3D-Medical-Images-Segmentation/result2023/swinunetr', output_ext=".nii.gz", output_postfix="gt",print_log=True)
    saver_seg = SaveImage(output_dir='/home/dluser/dataset/nnUNetFrame/Pytorch--3D-Medical-Images-Segmentation/result2023/swinunetr', output_ext=".nii.gz", output_postfix="seg",print_log=True)
    ...
    ckpt = torch.load(
        '/home/dluser/dataset/nnUNetFrame/Pytorch--3D-Medical-Images-Segmentation/result2023/checkpoint/best_metrich_0304_monai_swinunetr.pth')
    ...
    # optimizer.load_state_dict(ckpt["optim"]) # ckpt = 权重， [“ ”]存储自己定义的net还是loss还是epoch，需要指定好名字，不能写错
    # net.load_state_dict(ckpt["net"])
    # epoch = 0
        ...
        if (epoch + 1) % 5 == 0:
            torch.save(
                {"net": net.state_dict(),
                 ...
                 "epoch": epoch},
                os.path.join(opt.output_folder, 'latest_checkpoint_0305_monai_swinunetr.pth')
            )
                ...
                if dice > best_metric:
                    ...
                    torch.save(
                        { "net": net.state_dict(),
                          "optim": optimizer.state_dict(),
                          "epoch": epoch},
                        os.path.join(opt.output_folder, 'best_metric_0305_monai_swinunetr.pth')
                    )
                print('')
